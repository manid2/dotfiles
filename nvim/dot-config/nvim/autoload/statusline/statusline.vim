...
function! statusline#statusline#mode_flags()
	let l:md=mode(0)
	let l:ro=&readonly ? 'R' : ''
...
function! statusline#statusline#file_type()
	" TODO: truncate right when exceeding max chars.
	return "%0.10(".&filetype."%)"
endfunction

function! statusline#statusline#git_str()
	return "%0.24(".g:vim_git_branch."%)"
endfunction

function! statusline#statusline#current_tag()
	let l:ct=''
...
function! statusline#statusline#word_count()
	let l:wc=''
...
function! statusline#statusline#file_info()
	return "%0.50(%f:%l:%c%V|%L|%p%%%)"
endfunction

function! statusline#statusline#expr()
	let l:sp=' '
	let l:secsp="%="
	let l:slparts = [
...
function! statusline#statusline#toggle_word_count()
	if g:statusline_wordcount_disabled
