...
" Public function: Get git branch with status flags
function! statusline#git#branch_with_status() abort
...
" Private helper: Get git status flags (dirty, unpushed, unpulled)
...
" Private helper: Get git branch and status via git command
...
" Private helper: Get cached git directory
...
" Private helper: Get git operation label (MERGING, REBASING, etc.)
